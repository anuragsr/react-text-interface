...
export default class ManagerContent extends Component {
  ...
  showHoverPopover = e => {
    ...
    if (
      !isSelecting &&
      !isEditing &&
      !$(mainParent).find(".highlight").length
    ) {
      let el = $(e.target);
  ...
  handleSourceSugg = suggSources => {
    this.setState({
      suggSources,
      showCreateSource:
        this.state.source.name !== "" && suggSources.length === 0
    });
  };
  ...
  handleMouseDownOutside = e => {
    ...
    if (elLength === 1) {
      elementsToRemove.replaceWith(elementsToRemove.html());
    } else {
      elementsToRemove.each((idx, obj) => {
        let el = $(obj);
        if (idx === 0) el.replaceWith(el.html());
        else if (idx === elLength - 1) el.replaceWith(el.html());
  ...
  highlightSelection = sel => {
    ...
    rangy.createClassApplier("highlight", opts).applyToSelection();
  };
  ...
  openPopover = options => {
    ...
    if (options.edit) {
      ...
      switch (options.type) {
        case "tag":
          if (options.otherChosen) {
            // Copy object from other array
            tempSelObj = Object.assign(
              {},
              this.state.selEmoArr.filter(s => s.selId === hoverSelId)[0]
            );
            ...
            // Take existing object from own array
            tempSelObj = this.state.selTagArr.filter(
              s => s.selId === hoverSelId
            )[0];
        ...
        default:
          ...
          if (options.otherChosen) {
            ...
            tempSelObj = Object.assign(
              {},
              this.state.selTagArr.filter(s => s.selId === hoverSelId)[0]
            );
            tempSelObj.editType = "add";
            tempSelObj.noCreate = true;
            ...
            // Take existing object from own array
            tempSelObj = this.state.selEmoArr.filter(
              s => s.selId === hoverSelId
            )[0];
            tempSelObj.editType = "edit";
          }
  ...
  tagAdded = tag => {
    ...
    if (!tempTagArr.filter(t => t.id === tag.id).length) tempTagArr.push(tag);
      ...
      if (!emoArr.length) {
        ...
        elToRemove.each((idx, obj) => {
          ...
          if (
            prev.length &&
            next.length &&
            prev.attr("selid") === next.attr("selid")
          ) {
            el.remove();
      ...
      default:
        ...
        switch (tempSelObj.editType) {
          ...
          default:
            //case "edit"
            if (tempSelObj.emotion === 0) {
              let tagArr = this.state.selTagArr.filter(
                  s => s.selId === tempSelObj.selId
                ),
                selEmoArr = this.state.selEmoArr.filter(
                  s => s.selId !== tempSelObj.selId
                );
  ...
  createSelectionArea = type => {
    ...
    selEl.removeClass("highlight").addClass("mark");

    if (selEl.length > 1) {
      // Tag outside tag
      if (createInside) {
        // Tag outside tag, but inside bigger tag
        ...
        selElArr.forEach((obj, idx) => {
          ...
          let tempStr = "";
          if (idx === 0) {
            tempStr += `<span selid="${tempParent.attr(
              "selid"
            )}" class="mark">${childNodes[0].textContent}</span>`;
            ...
            if (childNodes[1])
              tempStr += `<span selid="${tempParent.attr(
                "selid"
              )}" class="mark">${childNodes[1].textContent}</span>`;
          } else {
            tempStr += `<span selid="${tempParent.attr(
              "selid"
            )}" class="mark">${childNodes[0].textContent}</span>`;
        ...
        selEl
          .eq(selEl.length - 1)
          .html(selEl.eq(selEl.length - 1).html() + "]");
      }
      ...
      if (createInside) {
        ...
        let tempStr = "";
        tempStr += `<span selid="${tempParent.attr("selid")}" class="mark">${
          childNodes[0].textContent
        }</span>`;
    ...
    selEl
      .toArray()
      .forEach(
        el => (caretPosEnd += el.textContent.replace(/\[|\]/g, "").length)
      );
  ...
  handleKeyUp = e => {
    if (!isTyping) {
      ...
      watchForText.forEach(selId => {
        ...
        let allEl = parent.find(`[selid="${selId}"]`),
          oldText,
          newText;
  ...
  processPlhInput = options => {
    ...
    if (options.type === "add") {
      // Add text from list or user input
      plh = options.plh;
      if (plh) {
        if (options.isClick) {
          currNode.textContent =
            nodeParts[0] + plh.slice(1, plh.length - 1) + "} " + nodeParts[1];
        } else {
        ...
        currNode.textContent =
          nodeParts[0] + "{" + currPlh + "} " + nodeParts[1];
      }
    } else {
      // Restore original
          ...
          try {
            if (plh) {
              if (options.isClick) {
                ...
                range.setStart(
                  currNode,
                  currPlhStart.focusOffset + plh.length + 1
                );
              ...
              range.setStart(
                currNode,
                currPlhStart.focusOffset + currPlh.length + 3
              );
            }
          } catch (err) {
            // l(err)
  ...
  submit = e => {
    ...
    if (this.state.upFile === "") {
      req = {
        ...
        text: $(this.contentEditable.current)
          .text()
          .replace(/\[|\]/g, ""),
        pq: this.state.pq,
        tags,
  ...
  render() {
    const placeUpOrDown = ({
        ...
        if (hoverEl) {
          ...
          if (style.top < frameTop || this.state.popPos === "down") {
            // Switch for up/down
            style.top = selectionTop + selectionHeight + gap;
          }
        }
    ...
    if (this.state.hoverEmo > 0) {
      switch (this.state.hoverEmo) {
        case 1:
          hoverEmoImg = "assets/emo-1.svg";
          break;
        case 2:
          hoverEmoImg = "assets/emo-2.svg";
          break;
        case 3:
          hoverEmoImg = "assets/emo-3.svg";
          break;
        case 4:
          hoverEmoImg = "assets/emo-4.svg";
          break;
        default:
          hoverEmoImg = "assets/emo-5.svg";
          break; //case 5
      }
    }
    ...
    return (
      <div className="manager-outer" onMouseDown={this.handleMouseDownOutside}>
        ...
        <div className="content">
          {this.state.showTxtErr && (
            <div className="txt-err">
              <img src="assets/bounds.svg" alt=" " />
              <span>Sorry. We don’t have any random texts at the moment.</span>
              <img
                onClick={() => this.setShowTextErr(false)}
                src="assets/bounds.svg"
                alt=" "
              />
            </div>
          )}

          {this.state.notifType === "greeting" && (
            <div className="notif-outer">
          ...
          {this.state.notifType === "submit" && (
            <div className="notif-outer">
              <div className="notif-inner">
                ...
                <button
                  className="btn-accent btn-anchor"
                  onClick={() => this.backToMain("undo")}
                  type="button"
                >
                  Undo
                </button>
              ...
              <div className="container">
                <div className="ctn-text ctn-source">
                  ...
                  {this.state.showCreateSource && (
                    <div className="ctn-no-sugg">
                      No suggested sources.{" "}
                      <a onClick={this.createSource} href="javascript:void(0)">
                        Click to Create Source
                      </a>
                    </div>
                  )}
                </div>
                ...
                <div className="ctn-text ctn-text-main" ref={this.ctnMainRef}>
                  ...
                  <SliderX
                    ...
                    style={{
                      display: this.state.ceContent === "" ? "none" : "block"
                    }}
                    ptq={this.state.pq}
                  ...
                  >
                    <div
                      onMouseDown={sp}
                      className={`ctn-pop-outer ${
                        this.state.popPos === "up"
                          ? "arrow-bottom"
                          : "arrow-top"
                      }`}
                    >
                      {this.state.popType === "" && (
                        <div className="ctn-btn-empty">
                          <img
                            onClick={() =>
                              this.openPopover({
                                type: "tag",
                      ...
                      {this.state.popType === "tag" && (
                        <div className="ctn-pop-inner ctn-tag">
                          ...
                          {this.state.tagStep === 1 &&
                            this.state.suggTags.length > 0 && (
                              <div className="ctn-sugg">
                          ...
                          {this.state.tagStep === 2 && (
                            <div className="ctn-ptq">
                      ...
                      {this.state.popType === "emoji" && (
                        <div className="ctn-pop-inner ctn-emo">
                          <img
                            onClick={e => this.closePopover(e, tmpHtml)}
                            className="close"
                            src="assets/bounds.svg"
                          ...
                          <div className="ctn-emo-inner">
                            ...
                            <div className="emo-outer">
                              <div
                                className={`emo-single ${
                                  this.state.tempSelObj.emotion === 1
                                    ? "active"
                                    : ""
                                }`}
                              ...
                              >
                                <img
                                  src="assets/emo-1.svg"
                                  alt=" "
                                  className="clr"
                                />
                                <img
                                  src="assets/emo-1-grey.svg"
                                  alt=" "
                                  className="bw"
                                />
                              </div>
                              <div
                                className={`emo-single ${
                                  this.state.tempSelObj.emotion === 2
                                    ? "active"
                                    : ""
                                }`}
                              ...
                              >
                                <img
                                  src="assets/emo-2.svg"
                                  alt=" "
                                  className="clr"
                                />
                                <img
                                  src="assets/emo-2-grey.svg"
                                  alt=" "
                                  className="bw"
                                />
                              </div>
                              <div
                                className={`emo-single ${
                                  this.state.tempSelObj.emotion === 3
                                    ? "active"
                                    : ""
                                }`}
                              ...
                              >
                                <img
                                  src="assets/emo-3.svg"
                                  alt=" "
                                  className="clr"
                                />
                                <img
                                  src="assets/emo-3-grey.svg"
                                  alt=" "
                                  className="bw"
                                />
                              </div>
                              <div
                                className={`emo-single ${
                                  this.state.tempSelObj.emotion === 4
                                    ? "active"
                                    : ""
                                }`}
                              ...
                              >
                                <img
                                  src="assets/emo-4.svg"
                                  alt=" "
                                  className="clr"
                                />
                                <img
                                  src="assets/emo-4-grey.svg"
                                  alt=" "
                                  className="bw"
                                />
                              </div>
                              <div
                                className={`emo-single ${
                                  this.state.tempSelObj.emotion === 5
                                    ? "active"
                                    : ""
                                }`}
                              ...
                              >
                                <img
                                  src="assets/emo-5.svg"
                                  alt=" "
                                  className="clr"
                                />
                                <img
                                  src="assets/emo-5-grey.svg"
                                  alt=" "
                                  className="bw"
                                />
                              </div>
                  ...
                  <div className="ctn-pop-hover arrow-bottom" ref={this.popRef}>
                    {this.state.hoverTagArr.length > 0 && (
                      <Tags
                    ...
                    <div className="ctn-pop-outer arrow-top">
                      <div className="ctn-plh">
                        <div className="plh-sugg-outer">
                          {this.state.filterPlhArr.length > 0 && (
                            <ul>
                              {this.state.filterPlhArr.map((item, i) => {
                                return (
                                  <li
                                    key={i}
                                    onClick={() =>
          ...
          <div className="ctn-buttons">
            <button
              className="btn-accent btn-sec"
              onMouseDown={this.getRandomText}
            >
              Get random text
            </button>
            <button className="btn-accent" onMouseDown={this.submit}>
              Submit
            </button>
          </div>
        </div>
